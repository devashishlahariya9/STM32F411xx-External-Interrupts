...
void NVIC_EnableIRQ(uint8_t _IRQ)
{
	if(_IRQ > 32)
	{
...
void NVIC_DisableIRQ(uint8_t _IRQ)
{
	if(_IRQ > 32)
	{
		NVIC_ISER -> ISER[(_IRQ / 32)] &= ~(1 << (_IRQ - 32));
...
void GPIO_EnableIRQ(uint8_t _port, uint8_t _pin, uint8_t _trigger)
{
...
void GPIO_clearPendingIRQ(uint8_t _pin)
{
	EXTI -> PR |= (1 << _pin);
}
